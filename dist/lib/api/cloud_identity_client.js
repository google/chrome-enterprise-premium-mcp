...
export class CloudIdentityClient {
  ...
  async createDlpRule(customerId, orgUnitId, ruleConfig, authToken) {
    logger.debug(
      `${TAGS.API} createDlpRule called with customerId: ${customerId}, orgUnitId: ${orgUnitId}, ruleConfig: ${JSON.stringify(ruleConfig)}`,
    )
    const client = await this.getPolicyClient(authToken)
    try {
      const request = {
  ...
  async createDetector(customerId, orgUnitId, detectorConfig, authToken) {
    logger.debug(
      `${TAGS.API} createDetector called with customerId: ${customerId}, orgUnitId: ${orgUnitId}, detectorConfig: ${JSON.stringify(detectorConfig)}`,
    )
    const client = await this.getPolicyClient(authToken)
    try {
      let detectorType
