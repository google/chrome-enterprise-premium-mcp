...
/**
 * The canonical presentation order for scope categories.
 * High-signal/important services (Chrome) are prioritized at the top
 * to help agents and users disambiguate this MCP from others.
 */
...
/**
 * Returns a sorted list of human-readable categories for the given scope URLs.
 * @param {string[]} scopes List of raw scope URLs.
 * @returns {string[]} List of unique category names in priority order.
 */
export function getScopeCategoriesList(scopes) {
  const requested = new Set()
  for (const url of scopes) {
    const meta = OAUTH_SCOPE_REGISTRY[url]
    if (meta?.category) {
      requested.add(meta.category)
    }
  }

  // Use the master order directly, filtered to only what's present in the input.
  return SCOPE_CATEGORY_ORDER.filter(cat => requested.has(cat))
}

/**
 * Returns a list of individual human-readable scope names.
 * @param {string[]} scopes List of raw scope URLs.
 * @returns {string[]} List of individual scope names, sorted alphabetically.
 */
export function getScopeNamesList(scopes) {
  return scopes.map(url => OAUTH_SCOPE_REGISTRY[url]?.name || url).sort((a, b) => a.localeCompare(b))
}
