...
export class CloudIdentityClient {
   ...
   * @returns {Promise<{client: object, source: string}>} The Cloud Identity Policies API context.
   */
  ...
  async _listPolicies(filter, authToken) {
    const { client: service, source } = await this.getPolicyClient(authToken)
    try {
      let nextPageToken
      const allPolicies = []
      do {
        ...
        const response = await callWithRetry(() => service.policies.list(request), 'policies.list')
      ...
      handleApiError(error, TAGS.API, 'listing policies', source)
    }
  }
  ...
  async createDlpRule(customerId, orgUnitId, ruleConfig, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(() => service.policies.create(request), 'policies.create')
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'creating DLP rule', source)
    }
  }
  ...
  async deleteDlpRulePreValidated(policyName, authToken) {
    ...
    const { client: service, source } = await this.getPolicyClient(authToken)
    try {
      const deleteResponse = await callWithRetry(() => service.policies.delete({ name: policyName }), 'policies.delete')
      return deleteResponse.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'deleting Chrome DLP rule', source)
    }
  }
  ...
  async createDetector(customerId, orgUnitId, detectorConfig, authToken) {
    ...
    try {
      ...
      handleApiError(error, TAGS.API, 'creating detector', source)
    }
  }
  ...
  async _getPolicy(policyName, typeDisplay, authToken) {
    ...
    const { client: service, source } = await this.getPolicyClient(authToken)
    try {
      const response = await callWithRetry(() => service.policies.get({ name: policyName }), 'policies.get')
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, `getting ${typeDisplay}`, source)
    }
  }
  ...
  async _deletePolicyWithValidation(policyName, validationFn, typeDisplay, authToken) {
    const { client: service, source } = await this.getPolicyClient(authToken)
    try {
      const getResponse = await callWithRetry(() => service.policies.get({ name: policyName }), 'policies.get')
      const policy = getResponse.data

      if (validationFn(policy)) {
        const deleteResponse = await callWithRetry(
          () => service.policies.delete({ name: policyName }),
          'policies.delete',
        )
        return deleteResponse.data
      } else {
      ...
      handleApiError(error, TAGS.API, `deleting ${typeDisplay} policy`, source)
    }
  }
}
