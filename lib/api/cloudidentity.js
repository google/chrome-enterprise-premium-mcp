...
async function getPolicyClient(authToken) {
    ...
    const authClient = await getAuthClient([SCOPES.CLOUD_IDENTITY_POLICIES], authToken)

    google.options({ auth: authClient })
    policyClient = google.cloudidentity(API_VERSIONS.CLOUD_IDENTITY)
 ...
 */
export async function listDlpPolicies(type, authToken, customerId) {
    if (!customerId) {
...
export async function createDlpRule(customerId, orgUnitId, ruleConfig, validateOnly = false, authToken) {
    ...
    try {
        ...
        console.error(`${TAGS.API} ✓ Successfully created DLP rule: ${response.data.name}`)
