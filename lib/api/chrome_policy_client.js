...
export class ChromePolicyClient {
   ...
   * @returns {Promise<{client: object, source: string}>} The Chrome Policy API context.
   */
  ...
  async resolvePolicy(customerId, orgUnitId, policySchemaFilter, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(() => service.customers.policies.resolve(request), 'policies.resolve')
      ...
      handleApiError(error, TAGS.API, 'resolving policy', source)
    }
  }
  ...
  async batchModifyPolicy(customerId, orgUnitId, requests, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(
        () => service.customers.policies.orgunits.batchModify(request),
        'policies.orgunits.batchModify',
      )
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'batch modifying policies', source)
    }
  }
}
