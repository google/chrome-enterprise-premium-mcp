...
export class AdminSdkClient {
   ...
   * @returns {Promise<{client: object, source: string}>} The Admin Directory service context.
   */
   ...
   * @param {string[]} scopes The scopes to request.
   * @param {string} authToken The OAuth 2.0 auth token.
   * @returns {Promise<{client: object, source: string}>} The Licensing service context.
   */
  ...
  async getCustomerId(authToken) {
    ...
    const { client: service, source } = await this.getAdminService(
    ...
    try {
      ...
      handleApiError(error, TAGS.API, 'retrieving customer ID', source)
    }
  }
  ...
  async listOrgUnits(options, authToken) {
    ...
    try {
      ...
      handleApiError(error, TAGS.API, 'listing organizational units', source)
    }
  }
  ...
  async listChromeActivities(options, authToken) {
    ...
    try {
      ...
      handleApiError(error, TAGS.API, 'listing Chrome activity logs', source)
    }
  }
  ...
  async checkCepSubscription(customerId, authToken) {
    ...
    const { client: service, source } = await this.getLicensingService(
      API_VERSIONS.LICENSING,
      [SCOPES.LICENSING],
      authToken,
    )
    try {
      ...
      handleLicensingError(error, 'checking CEP subscription', source)
    }
  }
  ...
  async checkUserCepLicense(userId, authToken) {
    ...
    try {
      ...
      handleLicensingError(error, `checking license for user ${userId}`, source)
    }
  }
}
 ...
 * @param {string} [source] - The credential source (e.g. 'adc', 'cache')
 * @throws {Error} Formatted error.
 */
function handleLicensingError(error, operationDescription, source) {
  if (error.response?.status === 403) {
    ...
    if (source) {
      finalError.credentialsSource = source
    }
    throw finalError
  }
  handleApiError(error, TAGS.API, operationDescription, source)
}
