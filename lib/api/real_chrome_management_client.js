...
export class RealChromeManagementClient extends ChromeManagementClient {
   ...
   */
  async getChromeManagementService(credential) {
   ...
   */
  async countBrowserVersions(customerId, orgUnitId, credential) {
    ...
    const client = await this.getChromeManagementService(credential)
    try {
   ...
   * @throws {Error} If customerId is missing or the API call fails.
   */
  async listCustomerProfiles(customerId, credential) {
