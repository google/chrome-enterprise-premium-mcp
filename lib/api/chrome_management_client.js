...
export class ChromeManagementClient {
  ...
  async queryContentTransfers(customerId, options = {}, authToken) {
    logger.debug(`${TAGS.API} queryContentTransfers called for customerId: ${customerId}`)
    const resolvedId = await this.resolveCustomerId(customerId, authToken)
    const client = await this.getClient(authToken)
    try {
      const url = this._getSecurityInsightsUrl(resolvedId, 'queryContentTransfers', 'v1alpha1')
      const response = await callWithRetry(
      ...
      handleApiError(error, TAGS.API, 'querying content transfers')
    }
  }
  ...
  async queryContentTransfersBreakdowns(customerId, options = {}, authToken) {
    logger.debug(`${TAGS.API} queryContentTransfersBreakdowns called for customerId: ${customerId}`)
    const resolvedId = await this.resolveCustomerId(customerId, authToken)
    const client = await this.getClient(authToken)
    try {
      const url = this._getSecurityInsightsUrl(resolvedId, 'queryContentTransfersBreakdowns', 'v1alpha1')
      const response = await callWithRetry(
        () => client.context._options.auth.request({ url, method: 'GET', params: options }),
      ...
      handleApiError(error, TAGS.API, 'querying content transfers breakdowns')
    }
  }
  ...
  async queryUrlVisits(customerId, options = {}, authToken) {
    logger.debug(`${TAGS.API} queryUrlVisits called for customerId: ${customerId}`)
    const resolvedId = await this.resolveCustomerId(customerId, authToken)
    const client = await this.getClient(authToken)
    try {
      const url = this._getSecurityInsightsUrl(resolvedId, 'queryUrlVisits', 'v1alpha1')
      const response = await callWithRetry(
        () => client.context._options.auth.request({ url, method: 'GET', params: options }),
      ...
      handleApiError(error, TAGS.API, 'querying URL visits')
    }
  }
  ...
  async queryUrlVisitsBreakdowns(customerId, options = {}, authToken) {
    logger.debug(`${TAGS.API} queryUrlVisitsBreakdowns called for customerId: ${customerId}`)
    const resolvedId = await this.resolveCustomerId(customerId, authToken)
    const client = await this.getClient(authToken)
    try {
      const url = this._getSecurityInsightsUrl(resolvedId, 'queryUrlVisitsBreakdowns', 'v1alpha1')
      const response = await callWithRetry(
        () => client.context._options.auth.request({ url, method: 'GET', params: options }),
      ...
      handleApiError(error, TAGS.API, 'querying URL visits breakdowns')
    }
  }
}
