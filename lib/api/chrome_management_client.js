...
export class ChromeManagementClient {
  ...
  async getClient(authToken) {
    return createApiClient(
      ...
      [
        ...
        SCOPES.CHROME_MANAGEMENT_SECURITYINSIGHTS_READONLY,
  ...
  async checkSecurityInsightsStatus(customerId, authToken) {
    ...
    const client = await this.getClient(authToken)
    try {
      const url = `https://chromemanagement.googleapis.com/v1/customers/${resolvedId}/enterprise/securityInsights:checkEnablementStatus`
      const response = await callWithRetry(
  ...
  async enableSecurityInsights(customerId, targetOus, authToken) {
    ...
    const client = await this.getClient(authToken)
    try {
      const url = `https://chromemanagement.googleapis.com/v1/customers/${resolvedId}/enterprise/securityInsights:enable`
      const body = {}
  ...
  async disableSecurityInsights(customerId, authToken) {
    ...
    const client = await this.getClient(authToken)
    try {
      const url = `https://chromemanagement.googleapis.com/v1/customers/${resolvedId}/enterprise/securityInsights:disable`
      const response = await callWithRetry(
