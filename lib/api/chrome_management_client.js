...
export class ChromeManagementClient {
   ...
   * @returns {Promise<{client: object, source: string}>} The Chrome Management service context.
   */
  ...
  async countBrowserVersions(customerId, orgUnitId, authToken) {
    ...
    const { client: service, source } = await this.getClient(authToken)
    try {
      ...
      const response = await callWithRetry(
        () => service.customers.reports.countChromeVersions(request),
        'reports.countChromeVersions',
      )
      return response.data.browserVersions
    } catch (error) {
      handleApiError(error, TAGS.API, 'counting browser versions', source)
    }
  }
  ...
  async listCustomerProfiles(customerId, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(() => service.customers.profiles.list(request), 'profiles.list')
      return response.data.chromeBrowserProfiles
    } catch (error) {
      handleApiError(error, TAGS.API, 'listing customer profiles', source)
    }
  }
  ...
  async checkSecurityInsightsStatus(customerId, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(
        () => service.context._options.auth.request({ url, method: 'GET' }),
        'enterprise.securityInsights.checkEnablementStatus',
      )
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'checking security insights status', source)
    }
  }
  ...
  async enableSecurityInsights(customerId, targetOus, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(
        () => service.context._options.auth.request({ url, method: 'POST', data: body }),
        'enterprise.securityInsights.enable',
      )
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'enabling security insights', source)
    }
  }
  ...
  async disableSecurityInsights(customerId, authToken) {
    ...
    try {
      ...
      const response = await callWithRetry(
        () => service.context._options.auth.request({ url, method: 'POST', data: {} }),
        'enterprise.securityInsights.disable',
      )
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'disabling security insights', source)
    }
  }
}
