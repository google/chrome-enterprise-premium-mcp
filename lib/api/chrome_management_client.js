...
export class ChromeManagementClient {
   ...
   * @private
   */
  _getSecurityInsightsUrl(resolvedId, action, apiVersion = 'v1') {
    const baseUrl = this.apiOptions.rootUrl || 'https://chromemanagement.googleapis.com/'
    const cleanBase = baseUrl.endsWith('/') ? baseUrl : `${baseUrl}/`
    return `${cleanBase}${apiVersion}/customers/${resolvedId}/enterprise/securityInsights:${action}`
  }
  ...
  async checkSecurityInsightsStatus(customerId, authToken) {
    ...
    try {
      const response = await callWithRetry(
        () =>
          client.customers.enterprise.securityInsights.checkEnablementStatus({
            customer: `customers/${resolvedId}`,
          }),
  ...
  async enableSecurityInsights(customerId, targetOus, authToken) {
    ...
    try {
      ...
      if (targetOus && targetOus.length > 0) {
        request.requestBody.targetOus = targetOus
      }
      const response = await callWithRetry(
  ...
  async disableSecurityInsights(customerId, authToken) {
    ...
    try {
      const response = await callWithRetry(
        () =>
          client.customers.enterprise.securityInsights.disable({
            customer: `customers/${resolvedId}`,
          }),
  ...
  /**
   * Helper method to perform Security Insights queries.
   * @param {string} customerId The customer ID or 'my_customer'.
   * @param {string} action The action suffix (e.g., 'queryContentTransfers').
   * @param {object} options Optional parameters.
   * @param {string} [authToken] Optional OAuth 2.0 access token.
   * @param {string} errorLabel Description of the operation for error logging.
   * @returns {Promise<object>} The query response.
   * @private
   */
  async _querySecurityInsights(customerId, action, options, authToken, errorLabel) {
    logger.debug(`${TAGS.API} ${action} called for customerId: ${customerId}`)
    const resolvedId = await this.resolveCustomerId(customerId, authToken)
    const client = await this.getClient(authToken)
    try {
      const url = this._getSecurityInsightsUrl(resolvedId, action, 'v1alpha1')
      const response = await callWithRetry(
        () => client.context._options.auth.request({ url, method: 'GET', params: options }),
        `enterprise.securityInsights.${action}`,
      )
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, errorLabel)
    }
  }
   ...
   * @param {string} customerId The customer ID or 'my_customer'.
   * @param {object} [options] Optional parameters.
   * @param {string} [options.filter] Optional filter (e.g., 'event_time >= ...').
   * @param {string} [authToken] Optional OAuth 2.0 access token.
   * @returns {Promise<object>} The query response containing summaries.
   */
  async queryContentTransfers(customerId, options = {}, authToken) {
    return this._querySecurityInsights(
      customerId,
      'queryContentTransfers',
      options,
      authToken,
      'querying content transfers',
    )
  }
   ...
   * @param {string} [options.filter] Optional filter.
   * @param {string} [options.metric] Optional metric.
   * @param {string} [options.breakdown] Optional breakdown dimension.
   * @param {string} [options.fixedTimeRange] Optional fixed time range.
   * @param {string} [authToken] Optional OAuth 2.0 access token.
   * @returns {Promise<object>} The query response.
   */
  async queryContentTransfersBreakdowns(customerId, options = {}, authToken) {
    return this._querySecurityInsights(
      customerId,
      'queryContentTransfersBreakdowns',
      options,
      authToken,
      'querying content transfers breakdowns',
    )
  }
   ...
   * @returns {Promise<object>} The query response containing summaries.
   */
  async queryUrlVisits(customerId, options = {}, authToken) {
    return this._querySecurityInsights(customerId, 'queryUrlVisits', options, authToken, 'querying URL visits')
  }
   ...
   * @returns {Promise<object>} The query response.
   */
  async queryUrlVisitsBreakdowns(customerId, options = {}, authToken) {
    return this._querySecurityInsights(
      customerId,
      'queryUrlVisitsBreakdowns',
      options,
      authToken,
      'querying URL visits breakdowns',
    )
  }
}
