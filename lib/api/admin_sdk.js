...
import { API_VERSIONS, SCOPES, TAGS, CURRENT_CUSTOMER } from '../constants.js'
 ...
 */
export async function listChromeActivities(options, credential, apiOptions = {}) {
 ...
 * @throws {Error} If the API call fails
 */
export async function listOrgUnits(options, credential, apiOptions = {}) {
  const service = await createApiClient(
    google.admin,
 ...
 * @throws {Error} If the API call fails
 */
export async function getCustomerId(credential, apiOptions = {}) {
  const service = await createApiClient(
    google.admin,
    API_VERSIONS.ADMIN_DIRECTORY,
