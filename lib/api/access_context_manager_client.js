...
export class AccessContextManagerClient {
   ...
   * @param {string} operationName Required. The name of the operation resource.
   * @param {string} [authToken] Optional OAuth 2.0 access token.
   * @returns {Promise<object>} The Operation resource.
   * @throws {Error} If the API call fails.
   */
  async getOperation(operationName, authToken) {
    logger.debug(`${TAGS.API} getOperation called for: ${operationName}`)
    if (!operationName) {
      throw new Error('Missing required parameter: operationName')
    }
    const client = await this.getClient(authToken)
    try {
      const response = await callWithRetry(() => client.operations.get({ name: operationName }), 'operations.get')
      return response.data
    } catch (error) {
      handleApiError(error, TAGS.API, 'getting operation')
    }
  }
   ...
   * @param {string} operationName Required. The name of the operation resource.
   * @param {string} [authToken] Optional OAuth 2.0 access token.
   * @param {number} [pollIntervalMs] Optional poll interval in milliseconds. Defaults to 2000.
   * @param {number} [timeoutMs] Optional timeout in milliseconds. Defaults to 60000.
   * @returns {Promise<object>} The response content of the completed operation.
   * @throws {Error} If the operation fails or times out.
   */
  async waitForOperation(operationName, authToken, pollIntervalMs = 2000, timeoutMs = 60000) {
    logger.debug(`${TAGS.API} waitForOperation starting for: ${operationName}`)
    const startTime = Date.now()
    while (Date.now() - startTime < timeoutMs) {
      const op = await this.getOperation(operationName, authToken)
      if (op?.done) {
        if (op.error) {
          const errMsg = op.error.message || 'Operation failed'
          const errCode = op.error.code ? ` (code: ${op.error.code})` : ''
          throw new Error(`Operation failed: ${errMsg}${errCode}`)
        }
        logger.debug(`${TAGS.API} Operation ${operationName} finished. Full operation:`, JSON.stringify(op, null, 2))
        logger.debug(`${TAGS.API} Operation ${operationName} response:`, JSON.stringify(op.response, null, 2))
        return op.response
      }
