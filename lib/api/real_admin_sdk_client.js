...
export class RealAdminSdkClient extends AdminSdkClient {
   ...
   */
  async getAdminService(version, scopes, credential) {
    return createApiClient(google.admin, version, scopes, credential, this.apiOptions)
  }
   ...
   */
  async getLicensingService(version, scopes, credential) {
    return createApiClient(google.licensing, version, scopes, credential, this.apiOptions)
  }
   ...
   */
  async getCustomerId(credential) {
   ...
   * @throws {Error} If the API call fails.
   */
  async listOrgUnits(options, credential) {
   ...
   * @throws {Error} If the API call fails.
   */
  async listChromeActivities(options, credential) {
   ...
   */
  async checkCepSubscription(customerId, credential) {
    ...
    const resolvedCustomerId =
      initialCustomerId === CURRENT_CUSTOMER ? (await this.getCustomerId(credential)).id : initialCustomerId

    const service = await this.getLicensingService(API_VERSIONS.LICENSING, [SCOPES.LICENSING], credential)
    try {
      const response = await callWithRetry(
        () =>
   ...
   * @throws {Error} If the API call fails or the Licensing API is not enabled.
   */
  async checkUserCepLicense(userId, credential) {
