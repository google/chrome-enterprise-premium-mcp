...
export class RealAdminSdkClient extends AdminSdkClient {
  ...
  async checkCepSubscription(customerId, authToken) {
    logger.debug(`${TAGS.API} checkCepSubscription called with customerId: ${customerId}`)
    const service = await this.getLicensingService(API_VERSIONS.LICENSING, [SCOPES.LICENSING_READONLY], authToken)
    try {
      const response = await callWithRetry(
        () =>
          service.licenseAssignments.listForProductAndSku({
            ...
            customerId: customerId || CURRENT_CUSTOMER,
          }),
