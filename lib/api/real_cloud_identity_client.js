...
export class RealCloudIdentityClient extends CloudIdentityClient {
   ...
   */
  async getPolicyClient(credential) {
   ...
   */
  async listDlpRules(credential) {
    logger.debug(`${TAGS.API} listDlpRules called`)
    return this._listPolicies(CLOUD_IDENTITY_FILTERS.DLP_RULE, credential)
  }
   ...
   */
  async listDetectors(credential) {
    logger.debug(`${TAGS.API} listDetectors called`)
    return this._listPolicies(CLOUD_IDENTITY_FILTERS.DETECTOR, credential)
  }
   ...
   * @private
   */
  async _listPolicies(filter, credential) {
    const client = await this.getPolicyClient(credential)
    try {
      let nextPageToken
      const allPolicies = []
      do {
   ...
   */
  async createDlpRule(customerId, orgUnitId, ruleConfig, credential) {
    logger.debug(
   ...
   */
  async deleteDlpRule(policyName, credential) {
   ...
   */
  async getDlpRule(policyName, credential) {
   ...
   */
  async getDetector(policyName, credential) {
   ...
   */
  async createDetector(customerId, orgUnitId, detectorConfig, credential) {
    logger.debug(
   ...
   * @returns {Promise<object>} The API response data.
   */
  async deleteDetector(policyName, credential) {
   ...
   * @private
   */
  async _deletePolicyWithValidation(policyName, validationFn, typeDisplay, credential) {
    const client = await this.getPolicyClient(credential)
    try {
