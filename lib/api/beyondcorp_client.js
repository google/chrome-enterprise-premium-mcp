...
export class BeyondCorpClient {
  ...
  /**
   * Updates a private web application.
   * @param {string} projectId The GCP project ID.
   * @param {string} gatewayId The secure gateway ID.
   * @param {string} applicationId The application ID.
   * @param {object} application The fields to update on the application.
   * @param {string} updateMask The update mask.
   * @param {string} [authToken] Optional authentication token.
   * @returns {Promise<object>} The updated application or LRO.
   */
  async patchApplication(projectId, gatewayId, applicationId, application, updateMask, authToken) {
    logger.debug(`${TAGS.API} patchApplication called for ${applicationId} on gateway ${gatewayId}`)
    const service = await this.getService(authToken)
    try {
      const response = await callWithRetry(
        () =>
          service.projects.locations.securityGateways.applications.patch({
            name: `projects/${projectId}/locations/global/securityGateways/${gatewayId}/applications/${applicationId}`,
            updateMask,
            requestBody: application,
          }),
      ...
      handleApiError(error, TAGS.API, `updating application ${applicationId}`)
    }
  }
