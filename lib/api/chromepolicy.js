 ...
 */
async function getChromePolicyClient(credential, apiOptions) {
 ...
 */
export async function getConnectorPolicy(customerId, orgUnitId, policySchemaFilter, credential, apiOptions = {}) {
  if (!customerId) {
    throw new Error('customerId is required for getConnectorPolicy')
  }

  const client = await getChromePolicyClient(credential, apiOptions)
