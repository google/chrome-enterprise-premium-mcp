...
export class RealServiceUsageClient extends ServiceUsageClient {
   ...
   */
  async getServiceUsageService(credential) {
   ...
   */
  async getServiceStatus(projectId, serviceName, credential) {
    logger.debug(`${TAGS.API} getServiceStatus called for ${serviceName} in project ${projectId}`)
    const service = await this.getServiceUsageService(credential)
    try {
   ...
   */
  async enableService(projectId, serviceName, credential) {
