 ...
 */
async function getManagementClient(credential, apiOptions) {
 ...
 */
export async function countBrowserVersions(customerId, orgUnitId, credential, apiOptions = {}) {
  if (!customerId) {
    throw new Error('customerId is required for countBrowserVersions')
  }

  const client = await getManagementClient(credential, apiOptions)
 ...
 * @throws {Error} If the API call fails
 */
export async function listCustomerProfiles(customerId, credential, apiOptions = {}) {
  if (!customerId) {
