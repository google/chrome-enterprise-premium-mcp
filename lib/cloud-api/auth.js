 ...
 * using API Key or the official Google Auth Library for Node.js. If GCP auth is not found, it logs an
...
export async function ensureGCPCredentials() {
  ...
  try {
    ...
    console.error(
      '1. Setting the `GOOGLE_API_KEY` environment variable to a valid API key.'
    );
    console.error(
      '2. If running locally, run: gcloud auth application-default login.'
    );
    console.error(
      '3. Ensuring the `GOOGLE_APPLICATION_CREDENTIALS` environment variable points to a valid service account key file.'
    );
    console.error(
      '4. If on a Google Cloud environment (e.g., GCE, Cloud Run), verify the associated service account has necessary permissions.'
    );
    console.error(
