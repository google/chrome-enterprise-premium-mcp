...
export function validateCelCondition(condition, triggers = []) {
  ...
  // Validate data type detectors inside matches_dlp_detector
  ...
  while ((dlpMatch = dlpDetectorRegex.exec(condition)) !== null) {
    ...
    if (extraParams) {
      errors.push(
        "The 'matches_dlp_detector' function does not support a second parameters argument. Please use 'matches_dlp_detector(\"DETECTOR_NAME\")' without extra arguments.",
      )
    }

    if (!DATA_TYPE_DETECTORS.includes(detector)) {
      errors.push(
        `'${detector}' is not a recognized data type DLP detector. To see a full list, ask me to list the data type detectors or check the dynamic reference.`,
      )
    }
  }
...
export function validateActionParameters(action, params = {}, triggers = []) {
  ...
  if (dataMasking) {
    if (!isUrlNavigationSelected) {
      ...
      // Backend restriction: only custom regular expressions and data masking detectors are supported for data masking (configured under regexDetectors)
      errors.push(ACTION_PARAMETER_CONSTRAINTS.DATA_MASKING_INVALID_PROPERTIES)
    }
  }
