...
export function validateCelCondition(condition, triggers = []) {
  ...
  while ((dlpMatch = dlpDetectorRegex.exec(condition)) !== null) {
    ...
    if (!DATA_TYPE_DETECTORS.includes(detector)) {
      errors.push(
        `'${detector}' is not a recognized data type DLP detector. To see a full list, ask me to list the predefined detectors.`,
      )
    }
  }
...
export function validateActionParameters(action, params = {}, triggers = []) {
  ...
  if (dataMasking) {
    if (!isUrlNavigationSelected) {
      ...
      // Backend restriction: only custom regular expressions and predefined detectors are supported for data masking (word lists and URL lists are not supported).
      errors.push(ACTION_PARAMETER_CONSTRAINTS.DATA_MASKING_SUPPORT)
    }
  }
