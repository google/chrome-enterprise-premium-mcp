...
export function validateMcpSafetyConstraints(action, state = POLICY_STATES.ACTIVE.value) {
  ...
  if (action === CHROME_ACTION_TYPES.BLOCK && state === POLICY_STATES.ACTIVE.value) {
...
export function validateActionParameters(action, params = {}, triggers = []) {
  ...
  if (customMessage) {
    ...
    // 1. Block any tag that isn't <a> or </a>
    const unauthorizedTags = customMessage.match(/<(?!\/?a(?=>|\s))[^>]+>/g)
    if (unauthorizedTags) {
      errors.push(ACTION_PARAMETER_CONSTRAINTS.CUSTOM_MESSAGE_SUPPORT)
    }

    // 2. For <a> tags, ensure only allowed attributes (href, target) are present.
  ...
  const isAdvancedActionSupported = action === CHROME_ACTION_TYPES.WARN || action === CHROME_ACTION_TYPES.AUDIT
