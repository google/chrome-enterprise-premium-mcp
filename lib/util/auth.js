/**
 * @fileoverview Authentication utilities for Google Cloud APIs.
 *
 * Provides functions to:
 * - Retrieve authenticated API clients.
 * - Verify Application Default Credentials (ADC).
 * - Generate helpful error messages for authentication failures.
 */

import { execFileSync } from 'node:child_process';
import { GoogleAuth, OAuth2Client } from 'google-auth-library';

import { TAGS, ERROR_MESSAGES } from '../constants.js';
...
function isGcloudInstalled() {
  try {
    // using execFileSync avoids shell execution and is safer
...
function suggestQuotaProject(errorMessage) {
  ...
  try {
    // 1. Try to get the current config project
    const configProject = execFileSync('gcloud', ['config', 'get-value', 'project'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
    if (configProject && configProject !== '(unset)') {
      return configProject;
    }

    // 2. Identify the API from the error message
    const apiMatch = errorMessage.match(/The ([a-z0-9.-]+) API requires/);
    const apiName = apiMatch ? apiMatch[1] : null;

    // 3. Get a list of active projects.
    // We do NOT use --sort-by=~createTime because it forces gcloud to fetch ALL projects 
    // before sorting, which is extremely slow for users in large organizations (10k+ projects).
    // Instead, we fetch a small batch (limit 10) of arbitrary active projects.
    const projectsOutput = execFileSync('gcloud', [
      'projects', 'list',
    ...
    // 4. If we know the API, check which project has it enabled
    if (apiName) {
      for (const projectId of candidates) {
        try {
          const serviceCheck = execFileSync('gcloud', [
            'services', 'list',
            '--project', projectId,
            '--enabled',
    ...
    // 5. Fallback: Return the most recent project if no specific match found
    return candidates[0];
  } catch {
