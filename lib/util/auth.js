...
/**
 * @file Authentication utilities for Google Cloud APIs.
 *
 * Provides functions to retrieve authenticated clients via Application Default
 * Credentials or OAuth tokens, verify access tokens, and enforce OAuth on
...
export async function verifyToken(accessToken, audience) {
  try {
    ...
    logger.error(`${TAGS.AUTH} Error verifying access token:`, error.message)
    throw error
  }
}
 ...
 * @throws {Error} Never throws directly, but logs authentication errors to the logger.
 */
export async function ensureADCCredentials() {
  ...
  try {
    ...
    logger.info(`${TAGS.AUTH} Authentication check for ADC failed (this is expected if not in GCP).`)
    logger.debug(getAuthErrorMessage(error))
    return false
  }
}
 ...
 * @returns {Promise<void>} Resolves once the request has been forwarded or a 401 has been sent.
 */
