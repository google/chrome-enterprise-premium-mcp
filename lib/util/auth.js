...
/**
 * Retrieves an authenticated Google Cloud client and its credential source.
 *
 ...
 * @returns {Promise<{client: import('google-auth-library').AuthClient, source: string}>} An object
 * containing the authenticated client instance and the source of credentials.
...
export async function getAuthClient(scopes, authToken) {
  ...
  try {
    ...
    // Verify the client is functional before returning it.
    // This catches cases where ADC is "configured" but invalid/expired
    // (e.g. gcloud credentials that require reauth).
    await client.getAccessToken()
    return { client, source: 'adc' }
  } catch (adcError) {
