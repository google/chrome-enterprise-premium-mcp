...
/**
 * Returns an authenticated client for Google API calls and its credential source.
 ...
 * @param {object} [_apiOptions] - Optional options containing status logging callbacks.
 * @returns {Promise<{client: import('google-auth-library').AuthClient, source: string}>} An object containing the client and its credential source.
 * @throws {Error} If no credential source resolves.
 */
export async function getAuthClient(scopes, authToken, _apiOptions = {}) {
  if (authToken) {
    const auth = new OAuth2Client()
    auth.setCredentials({ access_token: authToken })
    return { client: auth, source: 'bearer' }
  }

  const credPath = process.env.GOOGLE_APPLICATION_CREDENTIALS
  if (credPath) {
    const jwt = await jwtFromServiceAccountFile(credPath, scopes)
    return { client: jwt, source: 'adc' }
  }
  ...
  if (!tokens) {
    if (isStdioMode()) {
      throw new Error(
        `Authentication required. Run the \`cep_auth\` tool to sign in, or run \`${cliInvocation('auth login')}\` at the shell.`,
      )
    } else {
      throw new Error(
