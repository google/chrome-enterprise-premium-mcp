 ...
 * @param {string|string[]} status.scopes         ADC scope status label.
 * @param {string|string[]} status.oauthFlowScopes OAuth-flow scope status label.
 * @param {string|string[]} status.oauthClient    OAuth client configuration label.
 * @param {string|string[]} status.dataAccess     "Production" or "Fake".
 * @param {string|string[]} status.knowledge      Knowledge DB summary.
 */
export function printBanner({ transport, auth, apiCreds, scopes, oauthFlowScopes, oauthClient, dataAccess, knowledge }) {
  const rows = [
    ...
    '',
    `ADC scopes:      ${fmtField(scopes)}`,
    `OAuth flow:      ${fmtField(oauthFlowScopes)}`,
