 ...
 * @property {'adc'|'bearer-access'|'bearer-id'|'oauth-flow'} source The delivery channel that produced this probe.
 ...
 * @property {string[]} missingScopes The scopes the credential lacks against the required set.
 * @property {?Date} expiry                     Access-token expiry; null when not applicable.
 */

/**
 * @typedef {object} Credential
 * @property {() => Promise<CredentialProbe>} probe Returns a snapshot of the credential's current state.
 * @property {() => Promise<import('google-auth-library').AuthClient>} getClient Returns an auth client ready to make API calls.
 * @property {(probe: CredentialProbe, requiredScopes: string[]) => string[]|null} buildRemediation Returns remediation lines for the banner, or null when no action is needed.
 */

export { adcCredential } from './adc.js'
export { bearerCredential } from './bearer.js'
