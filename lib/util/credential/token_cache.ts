...
export class TokenCache {
   ...
   * @param filePath The cache file path.
   */
   ...
   * @returns The parsed token object, or null if file does not exist.
   */
   ...
   * persisting.
   * @param tokens The token object to persist.
   * @returns Resolves when the file has been written.
   */
  async write(tokens: Record<string, unknown>): Promise<void> {
    const safe = { ...tokens }
   ...
   * @returns True if the file mode is 0600, false otherwise.
   */
  async modeIsTight(): Promise<boolean> {
    try {
