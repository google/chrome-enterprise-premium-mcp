 ...
 * an in-memory factory never touch the native module. `@napi-rs/keyring` is an
 * optional dependency; when its native prebuild is missing (no libsecret on
 * Linux, headless CI, minimal Docker image), the import throws and is rewrapped
...
async function defaultKeyringFactory(service, account) {
  ...
  try {
    ...
    if (err && (err.code === 'MODULE_NOT_FOUND' || err.code === 'ERR_MODULE_NOT_FOUND')) {
      throw new Error(
        'Optional dependency `@napi-rs/keyring` is not installed (needs libsecret on Linux). ' +
          'Install it, or pass a custom `keyringFactory` via the options argument.',
        { cause: err },
      )
    }
    throw err
