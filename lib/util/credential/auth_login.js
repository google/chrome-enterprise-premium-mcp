...
const DEFAULT_AWAIT_CALLBACK_MS = 15_000
...
export async function startToolAuth({
  ...
  const callbackPromise = server.waitForCode().then(async r => {
    ...
    if (isErr) {
      ...
      return { kind: 'callback', ...r }
    }
    try {
      const tokens = await exchangeAndCache({ oauth2, code: r.code, codeVerifier, scopes, cachePath })
      if (pendingAuth === pending) {
        pendingAuth = null
        await pending.cancel()
      }
      return { kind: 'callback', ...r, tokens }
    } catch (err) {
      if (pendingAuth === pending) {
        pendingAuth = null
        await pending.cancel()
      }
      return { kind: 'callback', ...r, error: err.message }
    }
  })
  let timeoutHandle
  const timeoutPromise = new Promise(resolve => {
    // No browser opened means no callback can arrive, so we skip the wait to avoid process hang.
    const effectiveWaitMs = browserOpened ? awaitCallbackMs : 0
  ...
  if (winner.kind === 'callback') {
    ...
    return {
      ...
      expiresAt: winner.tokens?.expiry_date ? new Date(winner.tokens.expiry_date) : null,
    }
  }
 ...
 * @param {string} [opts.cachePath] Optional custom token cache path.
 ...
 */
export async function completeToolAuth({ redirectUrl, cachePath = TokenCache.defaultPath() }) {
  const validity = await isTokenLocallyValid({ cachePath })
  if (validity.ok) {
    return {
      status: 'completed',
