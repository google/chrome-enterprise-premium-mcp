 ...
 * @param {number} [opts._awaitCallbackMs] How long to wait for the callback before returning the paste-back path.
...
export async function startToolAuth({
  ...
  _awaitCallbackMs = readTimeoutMs(env),
} = {}) {
  ...
  // Start background listener for the callback. It runs asynchronously
  // and will exchange the token if it receives a callback before expiring.
  server
    .waitForCode()
    .then(async winner => {
      try {
        // Safety check: only proceed if this exact pending auth session is still active.
        if (pendingAuth !== pending) {
          return
        }
        if (winner.error) {
          logger.error('Background auth callback error:', winner.error)
          pendingAuth = null
          await pending.cancel()
          return
        }
        if (winner.state !== state) {
          logger.error('Background auth state mismatch')
          pendingAuth = null
          await pending.cancel()
          return
        }
        if (!winner.code) {
          logger.error('Background auth no code')
          pendingAuth = null
          await pending.cancel()
          return
        }
        await exchangeAndCache({ oauth2, code: winner.code, codeVerifier, scopes, cachePath })
        logger.info('Background auth completed successfully')
        pendingAuth = null
        await pending.cancel()
      } catch (err) {
        logger.error('Background auth exchange failed:', err?.message || err)
      }
    })
    .catch(err => {
      // Only log if it wasn't cancelled (stopping the server throws/rejects)
      if (pendingAuth === pending) {
        logger.error('Background auth waitForCode failed:', err)
      }
    })
