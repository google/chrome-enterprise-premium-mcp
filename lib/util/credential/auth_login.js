...
import { getActiveScopes, getDisabledScopes } from '../feature_flags.js'
...
export async function isTokenLocallyValid({
  ...
  const disabled = new Set(getDisabledScopes())
  scopes = scopes.filter(s => !disabled.has(s))
...
export async function startToolAuth({
  ...
  // Scopes are pre-filtered or custom passed
