...
export const FLAGS = {
  ...
  SECURITY_INSIGHTS_DATA_TOOL_ENABLED: 'SECURITY_INSIGHTS_DATA_TOOL_ENABLED',
  CLOUD_PLATFORM_SCOPE_ENABLED: 'CLOUD_PLATFORM_SCOPE_ENABLED',
  SECURE_GATEWAY_ENABLED: 'SECURE_GATEWAY_ENABLED',
  SEARCH_ORGANIZATIONS_TOOL_ENABLED: 'SEARCH_ORGANIZATIONS_TOOL_ENABLED',
  ACM_TOOLS_ENABLED: 'ACM_TOOLS_ENABLED',
}
...
const DEFAULT_VALUES = {
  ...
  [FLAGS.SECURITY_INSIGHTS_DATA_TOOL_ENABLED]: false,
  [FLAGS.CLOUD_PLATFORM_SCOPE_ENABLED]: false,
  [FLAGS.SECURE_GATEWAY_ENABLED]: false,
  [FLAGS.SEARCH_ORGANIZATIONS_TOOL_ENABLED]: false,
  [FLAGS.ACM_TOOLS_ENABLED]: false,
}
...
/**
 * Returns the array of currently disabled OAuth scopes based on feature flags.
 * @param {FeatureFlags} [flags] - Optional FeatureFlags instance to use.
 * @returns {string[]} Array of disabled scope URLs.
 */
export function getDisabledScopes(flags = featureFlags) {
  const disabled = []
  if (!flags.isEnabled(FLAGS.CLOUD_PLATFORM_SCOPE_ENABLED)) {
    disabled.push(SCOPES.CLOUD_PLATFORM)
  }
  return disabled
}

/**
 * Returns the active OAuth scope array based on current feature flag settings.
 * @param {FeatureFlags} [flags] - Optional FeatureFlags instance to use.
 * @returns {string[]} Array of active scope URLs.
 */
export function getActiveScopes(flags = featureFlags) {
  const disabled = new Set(getDisabledScopes(flags))
  return Object.values(SCOPES).filter(s => !disabled.has(s))
}
