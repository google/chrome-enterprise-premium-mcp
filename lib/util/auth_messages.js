...
/**
 * Builds the notice shown when credentials are valid but no quota project is
 * resolved. The OAuth-flow path picks the quota project up from
 * `GOOGLE_CLOUD_QUOTA_PROJECT`; without it, Google APIs reject calls as
 * "quota exceeded" or "API not enabled" even when no charges accrue.
 * @param {import('./credential/index.js').CredentialProbe & {quotaProject?: ?string}} probe - The credential probe result.
 * @returns {?string[]} Array of lines, or null when no notice is needed.
 */
export function buildQuotaProjectWarning(probe) {
  if (!probe.ok) {
    return null
  }
  if (probe.quotaProject) {
    return null
  }
  return [
    'No GCP quota project set. Google APIs use it to attribute quota and access checks.',
    'Set GOOGLE_CLOUD_QUOTA_PROJECT in your environment or .env file to a GCP project ID',
    'you have access to.',
  ]
}
