...
3. **Verify APIs are enabled:** Ask your Google Cloud administrator to enable the required APIs listed in lib/constants.js#SERVICE_NAMES.`
    ...
    // Fallback/Default to 'cache' (local OAuth flow or SA mode)
    const manualLogin = cliInvocation('auth login')
    if (resolvedStatus === 401) {
      if (isSaMode) {
        instruction =
          'Authentication required. The Service Account credentials configured in GOOGLE_APPLICATION_CREDENTIALS are invalid or domain-wide delegation failed. Ensure the Service Account JSON key is valid and domain-wide delegation (CEP_IMPERSONATE_SUBJECT) is configured in Google Workspace Admin Console.'
      } else {
      ...
      if (isSaMode) {
        instruction =
          'Permission denied. The Service Account lacks required Google Workspace / Chrome Enterprise permissions or domain-wide delegation OAuth scopes. Verify that the Service Account has required IAM roles and that Domain-Wide Delegation in Google Workspace Admin Console includes the necessary scopes.'
      } else {
