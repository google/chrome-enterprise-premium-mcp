...
let cachedAuthMode = /** @type {'oauth-flow'|'adc'|null} */ (null)
let authModeCheckPromise = /** @type {Promise<'oauth-flow'|'adc'>|null} */ (null)

/**
 * Detects whether the active credential is from the OAuth-flow cache rather
 * than ADC. Returns 'oauth-flow' when valid cached OAuth tokens are present,
 * 'adc' otherwise. Callers can override by passing `options.authMode`. The
 * result is memoized for the process lifetime — auth mode is effectively
 * immutable per process and the detection is on the error path, not the
 * happy path.
 * @returns {Promise<'oauth-flow'|'adc'>} The detected mode.
 */
function detectAuthMode() {
  if (cachedAuthMode !== null) {
    return Promise.resolve(cachedAuthMode)
  }
  if (!authModeCheckPromise) {
    authModeCheckPromise = (async () => {
      try {
        const cache = new TokenCache(TokenCache.defaultPath())
        const tokens = await cache.read()
        if (tokens && tokens.access_token) {
          cachedAuthMode = 'oauth-flow'
 ...
 * @param {'oauth-flow'|'adc'} [options.authMode] - Skip detection and force a remediation flavor.
...
export async function getAuthErrorMessage(error, options = {}) {
  ...
  if (authMode === 'oauth-flow') {
    const instruction = buildOAuthInstruction({ isInsufficientScopes, isNoCredentials, isQuotaProjectNotSet })
    if (instruction) {
  ...
  const instruction = await buildAdcInstruction({
    isInsufficientScopes,
    isNoCredentials,
    isQuotaProjectNotSet,
    gcloudInstalled,
