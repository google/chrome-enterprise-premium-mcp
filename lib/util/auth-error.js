 ...
 * errors and returns OAuth-flow remediation: `mcp auth login` and
...
export async function getAuthErrorMessage(error) {
  ...
  if (isInsufficientScopes) {
    ...
    instruction =
      'The API requires a quota project. Set `GOOGLE_CLOUD_QUOTA_PROJECT` in your environment or `.env` file to a GCP project ID you have access to.'
  }
