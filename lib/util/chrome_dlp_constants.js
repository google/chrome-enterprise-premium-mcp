...
export const ACTION_PARAMETER_CONSTRAINTS = {
  ...
  DATA_MASKING_SUPPORT:
    "The 'dataMasking' parameter is only supported with the 'URL_NAVIGATION' trigger and 'WARN' or 'AUDIT' actions. Currently, only custom regular expressions and masking detectors are supported for data masking (word lists and URL lists are not supported).",
  DATA_MASKING_INVALID_PROPERTIES:
    "Invalid dataMasking structure. The GCP backend only supports configuring masking detectors inside the 'regexDetectors' array (using custom regex policies/ resource names or lowercase shorthand masking detectors like 'cc-number'). Sibling properties like 'predefinedDetectors' or 'wordListDetectors' are not supported by the backend.",
}
