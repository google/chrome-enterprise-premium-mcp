...
export const CEL_FUNCTIONS = {
  ...
  "matches_dlp_detector('DETECTOR_NAME')":
    "Matches a data type DLP detector (e.g., 'US_SOCIAL_SECURITY_NUMBER', 'GERMANY_DRIVERS_LICENSE_NUMBER'). Do NOT include a second parameters argument, as it will cause an API error.",
  ...
  'Note for Detectors':
    "Data type detectors (used with 'matches_dlp_detector') use their canonical names (e.g. 'US_SOCIAL_SECURITY_NUMBER'). Custom detectors (regex, word list, URL list) use the full resource name (e.g., 'policies/abc-123') returned by list_detectors or detector creation tools.",
...
/**
 * Data type detector names for use with `matches_dlp_detector`.
 */
export const DATA_TYPE_DETECTORS = [
  'ADVERTISING_ID',
  'AGE',
...
export const MASKING_DETECTORS = [
  ...
  'common-currency-amount-suffix',
  'cc-number',
  'date',
  'email',
  'intl-phone-number',
  'ip-address',
  'payment-statement-link',
  'ssn',
  'time',
  '4-digit-cc-number',
]
...
export const ACTION_PARAMETER_CONSTRAINTS = {
  ...
  DATA_MASKING_SUPPORT:
    "The 'dataMasking' parameter is only supported with the 'URL_NAVIGATION' trigger and 'WARN' or 'AUDIT' actions. Currently, only custom regular expressions and data masking detectors are supported for data masking (word lists and URL lists are not supported).",
  DATA_MASKING_INVALID_PROPERTIES:
    "Invalid dataMasking structure. The GCP backend only supports configuring data masking detectors inside the 'regexDetectors' array (using custom regex policies/ resource names or lowercase shorthand data masking detectors like 'cc-number'). Sibling properties like 'predefinedDetectors' or 'wordListDetectors' are not supported by the backend.",
}
...
## 5. Data Type Detectors
Use these canonical strings exactly as shown with the \`matches_dlp_detector()\` function in the rule's trigger condition (CEL logic).
**CRITICAL:** These names (e.g., \`CREDIT_CARD_NUMBER\`) are strictly for trigger conditions and cannot be used in the \`dataMasking\` action parameters (they will result in a backend execution failure).

${DATA_TYPE_DETECTORS.join(', ')}.

---

## 5b. Data Masking Detectors
Use these shorthand lowercase strings exactly as shown when configuring the \`dataMasking\` action parameters (under \`resourceName\`).
**CRITICAL:** These shorthand names (e.g., \`cc-number\`) are strictly for the \`dataMasking\` action and cannot be used in the trigger condition CEL logic (e.g., \`matches_dlp_detector('cc-number')\` will fail CEL validation).

${MASKING_DETECTORS.join(', ')}.
