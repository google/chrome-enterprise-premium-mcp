...
export const PREDEFINED_MASKING_DETECTORS = [
...
Use these canonical strings exactly as shown with the \`matches_dlp_detector()\` function in the rule's trigger condition.
**CRITICAL:** These names (e.g., \`CREDIT_CARD_NUMBER\`) are strictly for trigger conditions and cannot be used in the \`dataMasking\` action parameters.

${PREDEFINED_DETECTORS.join(', ')}.

---

## 5b. Predefined Masking Detectors
...
${PREDEFINED_MASKING_DETECTORS.join(', ')}.
