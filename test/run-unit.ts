...
/**
 * @file Runs all local unit tests via the Node.js test runner with tsx import.
 *
 * Discovers `.test.ts` and `.test.js` files under `test/local/` and `test/unit/`
 * and runs them.
 */
...
try {
  // Use tsx loader to execute TypeScript test files dynamically in Node's runner
  execFileSync(process.execPath, ['--import', 'tsx', '--test', ...testFiles], {
    cwd: root,
    stdio: 'inherit',
  })
} catch {
