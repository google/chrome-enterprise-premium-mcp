...
describe('startToolAuth', () => {
  ...
  test('When the loopback callback fires with a matching state, then it exchanges the code and writes the cache in the background', async () => {
    ...
    // Trigger callback
    resolveCode({ code: 'good-code', state: receivedState })

    // Wait for background exchange
  ...
  test('When the loopback callback returns a mismatched state in the background, then it cancels the pending auth and does not write the cache', async () => {
    const { client } = makeFakeOAuth2Client()
    let resolveCode
    const codePromise = new Promise(resolve => {
      resolveCode = resolve
    ...
    // Trigger mismatched state
    resolveCode({ code: 'evil', state: 'attacker-state' })
    ...
    assert.strictEqual(server.wasStopped(), true)
  ...
  test('When the loopback callback returns access_denied in the background, then it cancels the pending auth and does not write the cache', async () => {
    const { client } = makeFakeOAuth2Client()
    let resolveCode
    const codePromise = new Promise(resolve => {
      resolveCode = resolve
    ...
    resolveCode({ error: 'access_denied' })
