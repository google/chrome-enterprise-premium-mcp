...
const SKIP_FILES = new Set(['index.js', 'index.ts', 'base-state.js', 'base-state.ts'])

/** @type {Map<string, (state: object) => object>} */
const registry = new Map()

const files = fs.readdirSync(__dirname).filter(f => (f.endsWith('.js') || f.endsWith('.ts')) && !SKIP_FILES.has(f))
for (const file of files) {
  const ext = file.endsWith('.js') ? '.js' : '.ts'
